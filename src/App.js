...
export default function EurovisionScoreboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-[#f0f4ff] to-[#f5f0ff] text-gray-900 font-sans flex flex-col items-center overflow-x-hidden relative text-center transition-all">
      
      {/* 1. ЭКСПОРТНЫЙ КОНТЕЙНЕР ( Stories 9:16 ) */}
      <div style={{ position: 'absolute', left: '-5000px', top: 0 }}>
        <div 
          ref={exportRef} 
          className="w-[1080px] h-[1920px] bg-gradient-to-br from-slate-50 via-[#f0f4ff] to-[#f5f0ff] pt-40 pb-40 px-16 flex flex-col items-center justify-between"
        >
          <header className="text-center flex flex-col items-center w-full mb-10">
            ...
            <div className="text-[#002FA7] text-6xl font-black uppercase tracking-[0.2em] italic">
              My Top 10
            </div>
          </header>
          ...
          <div className="flex flex-col gap-4 w-full px-2 flex-grow justify-center">
            {sorted.slice(0, 10).map((c, i) => (
              <div key={c.id} className="bg-white px-12 h-[110px] flex items-center justify-between rounded-[40px] border border-blue-100/50 shadow-sm">
                <div className="flex items-center gap-10 h-full">
                  ...
                  <div className="w-24 h-full flex items-center justify-center text-[70px] leading-none">
                    {c.flag}
                  </div>
                ...
                <div className="text-7xl font-black text-[#002FA7] flex items-center h-full pr-4 tracking-tighter leading-none">
                   {c.score}
                </div>
              </div>
            ))}
      ...
      {/* 2. ПАНЕЛЬ УПРАВЛЕНИЯ */}
      {!isExporting && (
        <div className="w-full bg-white/60 backdrop-blur-md border-b py-3 px-3 sm:px-6 flex justify-center gap-2 sm:gap-3 sticky top-0 z-50 shadow-sm">
          {votingStarted ? (
            <>
              <button onClick={resetScores} className="px-3 sm:px-4 py-2 bg-red-50/50 text-red-600 rounded-lg text-xs sm:text-sm font-bold border border-red-100 transition-colors whitespace-nowrap">Reset</button>
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap ${
                  votesCount === 10 ? "bg-[#002FA7] text-white shadow-md" : "bg-gray-100/50 text-gray-400 cursor-not-allowed"
                }`}
              >
      ...
      {/* 3. ОСНОВНОЙ СПИСОК ( Glassmorphism Effect ) */}
      <div className="w-full max-w-5xl p-3 sm:p-4 md:p-8 flex flex-col relative z-10">
        <header className="mb-6 md:mb-8 text-center flex flex-col items-center">
          <img src={logoUrl} crossOrigin="anonymous" className="h-16 sm:h-20 md:h-28 mb-3 object-contain" alt="Logo" />
            ...
            <React.Fragment key={c.id}>
              {votingStarted && i === 10 && (
                <div className="w-full py-10 flex items-center gap-4 px-2 font-sans text-center">
                  ...
                  <span className="text-gray-400 text-[10px] md:text-xs font-black uppercase tracking-[0.3em] whitespace-nowrap shrink-0">Zero Points</span>
                  <div className="flex-1 h-px bg-gray-300"></div>
                </div>
              )}
              <motion.div layout className={`bg-white/70 backdrop-blur-md border p-2.5 sm:p-3 md:pl-5 md:pr-6 flex flex-col md:flex-row items-stretch md:items-center justify-between shadow-sm rounded-lg gap-3 ${c.score > 0 ? "border-[#002FA7]/40 ring-1 ring-[#002FA7]/10" : "border-white/50"}`}>
                <div className="flex items-start sm:items-center md:items-center gap-2 sm:gap-4 md:gap-6 w-full md:w-auto">
                  ...
                  <div className="text-2xl sm:text-3xl md:text-4xl flex items-center justify-center shrink-0 leading-normal rounded-full bg-white/50 w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 shadow-inner">{c.flag}</div>
                  <img src={c.image} alt="" className="w-14 h-9 sm:w-16 sm:h-10 md:w-24 md:h-14 rounded-md object-cover bg-gray-100/50 shrink-0 shadow-sm" />
                  <div className="flex-1 min-w-0 md:w-56 ml-0 md:ml-4">
                    ...
                    <textarea
                      ...
                      className="w-full mt-2 p-1.5 text-[10px] bg-white/50 border border-transparent border-b-gray-200 rounded text-gray-400 focus:text-gray-700 outline-none resize-none transition-all focus:border-[#002FA7]/30"
                      rows="1"
                    />
                  </div>
        ...
        <footer className="mt-12 mb-4 text-center text-gray-400 text-xs font-medium uppercase tracking-[0.2em] opacity-60 relative z-20">
