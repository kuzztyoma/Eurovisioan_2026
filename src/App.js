...
export default function EurovisionScoreboard() {
  ...
  const [countries, setCountries] = useState(() => {
    const saved = localStorage.getItem("eurovision-scores-2026-v17");
    return saved ? JSON.parse(saved) : initialCountries;
  });
  const [isExporting, setIsExporting] = useState(false);

  useEffect(() => {
    localStorage.setItem("eurovision-scores-2026-v17", JSON.stringify(countries));
  }, [countries]);
  ...
  const resetScores = () => {
    if (window.confirm("Reset all votes?")) {
      ...
      localStorage.removeItem("eurovision-scores-2026-v17");
    }
  };
  ...
  const downloadAsImage = async () => {
    ...
    }, 1000); // Чуть больше времени на прогрузку шрифтов
  };
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-[#6366f1]/15 via-slate-50 to-[#d8b4fe]/15 text-gray-900 font-sans flex flex-col items-center overflow-x-hidden relative text-center">
      ...
      {/* 1. ЭКСПОРТНЫЙ КОНТЕЙНЕР (Чистый, без теней для исключения шашечек) */}
        ...
        <div ref={exportRef} className="w-[1080px] h-[1920px] bg-[#f3f4f6] pt-40 pb-40 px-16 flex flex-col items-center justify-between">
          ...
          <div className="flex flex-col gap-4 w-full px-2 flex-grow justify-center">
            {sorted.slice(0, 10).map((c, i) => (
              <div key={c.id} className="bg-white px-12 h-[110px] flex items-center justify-between rounded-[40px] border border-gray-200">
                <div className="flex items-center gap-10 h-full">
                  ...
                  <div className="ml-2 text-left flex flex-col justify-center h-full">
                    <h2 className="text-[44px] font-black uppercase tracking-tighter leading-none mb-1 text-gray-900">{c.name}</h2>
      ...
      {/* 3. ОСНОВНОЙ СПИСОК (Плавная анимация без рывков) */}
        ...
        <div className="flex flex-col gap-5 w-full">
          {sorted.map((c, i) => (
            <React.Fragment key={c.id}>
              ...
              {/* Убран transition-all для предотвращения конфликта с layout-анимацией */}
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 flex-1 w-full md:w-auto min-w-0 h-full">
                    ...
                    <div className="flex flex-col text-center md:text-left flex-1 min-w-0 w-full justify-center">
                      ...
                      <textarea 
                        ...
                        className="w-full mt-1 p-3 text-xs bg-white/60 border border-slate-200 rounded-2xl text-gray-800 focus:bg-white/90 focus:border-[#002FA7]/30 outline-none resize-none placeholder:text-gray-500 shadow-inner md:max-w-xs lg:max-w-md h-9 transition-colors" 
                        rows="1" 
                      />
                    </div>
                  </div>
                    ...
                    {[1, 2, 3, 4, 5, 6, 7, 8, 10, 12].map((p) => {
                      ...
                      return (
                        <button 
                          ...
                          className={`h-10 w-full md:w-10 flex items-center justify-center rounded-xl text-xs font-black transition-all border ${
                            mine 
